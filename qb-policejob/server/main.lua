...
local function UpdateBlips()
    ...
    for k, v in pairs(players) do
        if (v.PlayerData.job.type == "leo" or v.PlayerData.job.name == "ambulance") and v.PlayerData.job.onduty then
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
        if args[2] == "driver" or args[2] == "weapon" or args[2] == "hunting" or args[2] == "fishing" or args[2] == "pilot" then
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {{name = "id", help = Lang:t('info.player_id')}, {name = "time", help = Lang:t('info.jail_time')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
        ...
        if time > 0 and OtherPlayer then
            TriggerClientEvent("police:client:SendToJail", playerId, time)
            exports['qb-jail']:jailPlayer(playerId, true, time)
        else
            TriggerClientEvent('QBCore:Notify', src, 'Time must be larger than 0 or invalid Player ID', 'error')
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_police_only"), 'error')
    end
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if ((Player.PlayerData.job.type == "leo") and Player.PlayerData.job.onduty) and OtherPlayer then
...
QBCore.Commands.Add("gsr", "Test a player for GSR (gsr test needed) (Police Only)", {{name = "id", help = "ID of the person"}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(tonumber(args[1]))
    ...
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
        if Player.Functions.RemoveItem("pd_gsrtest", 1) then
            TriggerClientEvent('evidence:client:gsrtest', OtherPlayer.PlayerData.source, src)
        else
            TriggerClientEvent('QBCore:Notify', src, "You must have a gsr test on you..", "error")
        end
    end
end)
...
QBCore.Functions.CreateUseableItem("filled_evidence_bag", function(source, item)
    ...
    if Player.Functions.GetItemByName(item.name) then
        if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
            TriggerClientEvent('police:client:CheckEvidenceBag', src, item.info)
        end
    end
end)

QBCore.Functions.CreateUseableItem("pd_spikestrip", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(item.name) then
        if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Functions.CreateUseableItem("pd_emergencybutton", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.Functions.GetItemByName(item.name) then
        if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
            TriggerClientEvent('qb-policejob:client:emergencybutton', src)
        end
    end
end)
...
QBCore.Functions.CreateCallback('police:server:IsPoliceForcePresent', function(source, cb)
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for k, v in pairs(players) do
        if v.PlayerData.job.type == "leo" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
RegisterNetEvent('police:server:CuffPlayer', function(playerId, isSoftcuff)
    ...
    if CuffedPlayer then
        if Player.Functions.GetItemByName("handcuffs") or Player.PlayerData.job.type == "leo" then
            TriggerClientEvent("police:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
            TriggerEvent("qb-log:server:CreateLog", "cuffing", CuffedPlayer.PlayerData.name.." is cuffed", "red", "**".. Player.PlayerData.name .. "** (citizenid "..Player.PlayerData.citizenid..") has cuffed ".. CuffedPlayer.PlayerData.name)
        end
    end
end)
...
RegisterNetEvent('police:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    ...
    if IsVehicleOwned(plate) then
        if not fullImpound then
            MySQL.update('UPDATE player_vehicles SET state = ?, depotprice = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?', {
                0, 
                price, 
                body, 
                engine, 
            ...
            MySQL.update('UPDATE player_vehicles SET state = ?, body = ?, engine = ?, fuel = ? WHERE plate = ?', {
                2, 
                body, 
                engine, 
                fuel, 
...
RegisterNetEvent('evidence:server:AddCasingToInventory', function(casingId, casingInfo)
    ...
    if Player.Functions.RemoveItem("empty_evidence_bag", 1) then
        if Player.Functions.AddItem("filled_evidence_bag", 1, false, casingInfo) then
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items["filled_evidence_bag"], "add", 1)
...
RegisterNetEvent('evidence:server:gsrtest', function(playerId, result)
    local src = source
    TriggerClientEvent('chat:addMessage', playerId,  {
        template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}</strong><br><br> <strong>ID:</strong> {1} <br><strong>Result:</strong> {2}</div></div>',
        args = {'GSR Test', src, result}
    })
end)

RegisterNetEvent('police:server:takeoffmask', function(playerId)
    local src = source
    TriggerClientEvent('police:client:removemask', playerId)
end)

RegisterNetEvent('police:server:CheckSerial', function(serial)
    if not serial then return end
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end

    local result = MySQL.query.await('SELECT * FROM weapon_serials WHERE serial = ?', {serial})
    if result[1] then
        TriggerClientEvent('chat:addMessage', src,  {
            template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}</strong><br><br> <strong>Serial:</strong> {1} <br><strong>Weapon:</strong> {2} <br><strong>Name:</strong> {3} <br><strong>Citizen ID:</strong> {4}</div></div>',
            args = {
                'Weapon Registration', 
                result[1].serial, 
                result[1].weapon, 
        ...
        TriggerClientEvent('chat:addMessage', src,  {
            template = '<div class="chat-message advert"><div class="chat-message-body"><strong>{0}</strong><br><br> <strong>Serial:</strong> {1}</div></div>',
            args = {
                'Weapon Registration', 
