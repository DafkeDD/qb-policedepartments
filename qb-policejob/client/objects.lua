...
RegisterNetEvent('police:client:SpawnSpikeStrip', function()
    ...
    if #SpawnedSpikes + 1 <= Config.MaxSpikes then
        if PlayerJob.type == "leo" and PlayerJob.onduty then
            QBCore.Functions.Progressbar("spawn_object", "Placing Spike Strip", 1000, false, true, {
                disableMovement = true,
                disableCarMovement = true,
                disableMouse = false,
                disableCombat = true,
                ...
                StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
                TriggerServerEvent("QBCore:Server:RemoveItem", "pd_spikestrip", 1, false)
                TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["pd_spikestrip"], "remove", 1)
                    ...
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.type == "leo" and PlayerJob.onduty then
                            ...
                            if IsControlJustPressed(0, 38) then
                                ...
                                TriggerServerEvent('police:server:SyncSpikes', SpawnedSpikes)
                                Wait(250)
                                TriggerServerEvent("QBCore:Server:AddItem", "pd_spikestrip", 1, false)
                                TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items["pd_spikestrip"], "add", 1)
                            end
                        end
                    end
                end
