...
RegisterNetEvent('evidence:client:gsrtest', function(requestId)
    local retval = "Negative"
    if CurrentStatusList["gunpowder"] then
        if CurrentStatusList["gunpowder"].time > 0 then
            retval = "Positive"
        end
    end
    TriggerServerEvent('evidence:server:gsrtest', requestId, retval)
end)

RegisterNetEvent('police:client:CheckEvidenceBag', function(itemInfo)
    if itemInfo.type == 'casing' then
        QBCore.Functions.Progressbar('check_casing', "Looking up registration..", 24000, false, true, {
            disableMovement = false,
            disableCarMovement = false,
            disableMouse = false,
            disableCombat = true
        }, {}, {}, {}, function() -- Done
            TriggerServerEvent('police:server:CheckSerial', itemInfo.serie)
        end, function() -- Cancel
    ...
    while true do
        ...
        if IsPedShooting(ped) then
            ...
            if not WhitelistedWeapon(weapon) then
                ...
                if shotAmount > 0 and (CurrentStatusList == nil or CurrentStatusList['gunpowder'] == nil) then
...
CreateThread(function()
    while true do
        ...
        if CurrentCasing and CurrentCasing ~= 0 then
            local pos = GetEntityCoords(PlayerPedId())
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 3.5 then
        ...
        if CurrentBlooddrop and CurrentBlooddrop ~= 0 then
            ...
            if #(pos - vector3(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y,
                Blooddrops[CurrentBlooddrop].coords.z)) < 3.5 then
        ...
        if CurrentFingerprint and CurrentFingerprint ~= 0 then
            ...
            if #(pos - vector3(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y,
                Fingerprints[CurrentFingerprint].coords.z)) < 3.5 then
