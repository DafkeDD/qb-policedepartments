...
RegisterNetEvent('police:client:GetCuffed', function(playerId, isSoftcuff)
    ...
    if not isHandcuffed then
        local seconds = math.random(6, 9)
        local success = nil

        if attempts < 3 then
            local serverId = GetPlayerServerId(PlayerId())
            if not IsTargetDead(serverId) then
                success = exports['qb-lock']:StartLockPickCircle(1, seconds, success)
            else
                success = false
            end
        end
        ...
        if not success then
            ...
            CreateThread(function()
                Wait(60*1000)
                attempts -= 1
                if attempts < 0 then attempts = 0 end
            end)
        end
    else
        isHandcuffed = false
